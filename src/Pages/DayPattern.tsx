...
import { AnalysisTypes, DayPatternAnalysisTypes, useDocumentTitle } from '../utils/Helpers';
...
export default function DayPattern(): JSX.Element {
    ...
    const [currAnalysisType, setCurrAnalaysisType] = useState<analysisLevel>(DayPatternAnalysisTypes[0]);
    ...
    const handleTripSelectionChange = useCallback((newSelections: DayPatternSelections) => {
        setCurrAnalaysisType(prev => {
            if (newSelections.analysisTypeValue != undefined)
                return newSelections.analysisTypeValue;
            else return prev;
    ...
    useEffect(() => {
        console.log(currAnalysisType);
    }, [currAnalysisType]); // Add dependencies
    ...
    return (
        <div className="app-layout">
            ...
            {
                (
                    (currAnalysisType === DayPatternAnalysisTypes[0] && isWithinYearLoading) ||
                    (currAnalysisType === DayPatternAnalysisTypes[2] && isCrossSegmentLoading) ||
                    (currAnalysisType === DayPatternAnalysisTypes[1] && isBtwYearLoading)
                ) && <LoadingOverlay />
            }

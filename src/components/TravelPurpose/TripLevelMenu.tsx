...
import { WeekOptions, TripLevelDataProvider, AnalysisLevels, AnalysisTypes,TripPurposeOptions } from '../../utils/Helpers';
...
const TripLevelMenu: React.FC<{ onSelectionChange: (tripSelections: { week: weekOption, analysisLevelValue: analysisLevel, analysisTypeValue: analysisType, includeDecember: boolean, analysisYear: string}) => void,}> = ({ onSelectionChange }) => {
    ...
    const [analysisLevelValue, setAnalysisLevelValue] = useState<analysisLevel>(AnalysisLevels[1]); // Default analysis level
    const [analysisTypeValue, setAnalysisTypeValue] = useState<analysisType>(AnalysisTypes[0]); // Default analysis type
    ...
    useEffect(() => {
        const cacheKey = "TripLevelYearDataCache";

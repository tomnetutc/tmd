...
export const prepareVerticalChartData = (
  ...
  const labelPurposeDistribution = TripLevelTripPurposeOptions.filter(
    (option) => option.label !== "All"
  ).map((option) => `${option.label}`);
  const mapModeToIndex = TripLevelTripPurposeOptions.filter(
    (option) => option.label !== "All"
  ...
  filteredData.forEach((dataRow) => {
    ...
    optionValues.forEach((option) => {
      ...
      if (
        dataRow.tr_mode === option.numberTrip ||
        ...
        if (!TripDurationPerOption[option.numberTrip]) {
          ...
          TripPurposeDistributionPerOption[option.numberTrip] = new Array(
            labelPurposeDistribution.length
          ).fill(0);
        }
        ...
        TripPurposeDistributionPerOption[option.numberTrip][
          mapModeToIndex[purpose]
        ] += 1;
      }
    });
  });
  ...
  Object.keys(TripDurationPerOption).forEach((key: string) => {
    ...
    totalPurposeDistributionRows[key] = TripPurposeDistributionPerOption[
      key
  ...
  Object.keys(TripDurationPerOption).forEach((key: string) => {
    ...
    if (totalPurposeDistributionRows[key] > 0) {
      TripPurposeDistributionPerOption[key] = TripPurposeDistributionPerOption[
        key
      ].map(
        (value: number) => (value / totalPurposeDistributionRows[key]) * 100
      );
    }
  });
